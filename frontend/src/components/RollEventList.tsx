...
function RollEventEdit({ event, setEvents, onDone, onDelete }:
    ...
    const handleTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        updateEvent({ type: e.target.value as EventType });
    };

    const handleTagChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        updateEvent({ tag: e.target.value || undefined });
    };
        ...
        <div className="mt-4">
            ...
            {events.length === 0 ? (
                ...
                <ul className="max-h-48 overflow-y-auto space-y-1 text-sm">
                    {events.map((event) => (
                        <li className="flex items-center gap-2 p-1 rounded hover:bg-gray-100" key={event.key}
