...
CAM_TOPIC = '/SC/debug/annotated_camera_frame'


def read_bag(path):
    out = {'odom': [], 'llh': [], 'gnss2': [], 'imu': [], 'status': [], 'tf': {}, 'cam_ts': []}
    topics = ('/ekf/odometry_earth', '/ekf/llh_position', '/gnss_2/llh_position',
              '/imu/data', '/ekf/status', '/tf_static')
    with AnyReader([path]) as reader:
        conns = [c for c in reader.connections if c.topic in topics + (CAM_TOPIC,)]
        ...
        for conn, ts, raw in reader.messages(connections=conns):
            ...
            if conn.topic == CAM_TOPIC:
                out['cam_ts'].append(ts)  # header stamps are zero; bag receive time is epoch
                continue
...
def main():
    ...
    if svo is None:
        if not bag['cam_ts']:
            sys.exit(f'no vid.svo2 and no {CAM_TOPIC} frames in the bag')
    ...
    frame_ts = svo['frame_ts'] if svo is not None else np.array(bag['cam_ts'])
    vt = frame_ts / 1e9 + offset
    ...
    if svo is not None:
        ...
        camera = {'source': 'svo2', 'serial': svo['serial'], 'width_sbs': width, 'height': height,
        ...
        sync_method = 'none' if args.no_sync else 'gyro_xcorr'
    else:
        camera = {'source': 'bag_jpeg', 'topic': CAM_TOPIC, 'num_frames': len(frame_ts),
                  'video_t_start_ns': int(frame_ts[0]), 'video_t_end_ns': int(frame_ts[-1])}
        sync_method = 'bag_clock'
